...
# Times are printed in this PC's local time zone (no tzdata needed on Windows).
TZ = None
...
def when(iso):
    if not iso:
        return "—"
    return datetime.fromisoformat(iso.replace("Z", "+00:00")).astimezone().strftime("%d/%m/%Y %H:%M:%S")
