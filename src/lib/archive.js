...
function dayRecord(missionDay) {
  ...
  return {
    ...
    habitat, entries, moods, messages, traffic, sealed: !!sealed,
    isEmpty: !day && !entries.length && !messages.length && !habitat.length,
  };
}
...
function dayMarkdown(missionDay) {
  ...
  if (notes.length) {
    ...
    for (const n of notes) out.push(`- **${n.kind}** — ${n.body}`);
    out.push('');
  }
