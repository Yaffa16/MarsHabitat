...
const CFG = {
  ...
  // The record closes the day after the run ends: from the end of 28 October
  // 2026 (Berlin, winter time) the node is not polled again and what is
  // stored is the record. CRITICAL_FREEZE_AT overrides it for a rehearsal.
  freezeAt: Date.parse(process.env.CRITICAL_FREEZE_AT || '2026-10-28T23:59:59+01:00'),
};
...
async function poll() {
  if (frozen()) {
    ...
    console.log('[critical] the record is closed — the node is not polled after 28 October 2026');
    return;
  }
