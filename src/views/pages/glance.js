...
const tile = (label, unit, av, lo, hi, n, dec) => `<div class="glance-tile">
    <span class="gt-label">${esc(label)}</span>
    <span class="gt-value">${av == null ? '—' : Number(av).toFixed(dec)}<em>${esc(unit)}</em></span>
    <span class="gt-range">${lo == null ? '' : `${Number(lo).toFixed(dec)}–${Number(hi).toFixed(dec)} · ${n} readings`}</span>
  </div>`;
 ...
 */
function dayChart(s, dayStart, label, unit, dec, tz) {
  ...
  const dots = pts.map((p) => `<circle class="gc-dot" cx="${x(p.t).toFixed(1)}" cy="${y(p.v).toFixed(1)}" r="2.4"><title>${hm(p.t)} habitat time · ${f(p.v)} ${esc(unit)}</title></circle>`).join('');
  const first = pts[0], last = pts[pts.length - 1];
  return `<div class="glance-chart">
    <div class="gc-head"><b>${esc(label)}</b><span>${pts.length} reading${pts.length === 1 ? '' : 's'} · ${f(Math.min(...pts.map((p) => p.v)))}–${f(Math.max(...pts.map((p) => p.v)))} ${esc(unit)} · first ${hm(first.t)} · last ${hm(last.t)} habitat time</span></div>
    <svg class="gc-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(label)}: every reading of the day">
...
function daySection(r, m, { rehearsal = false } = {}) {
  ...
  const head = `
    <header class="glance-head">
      <div>
        <span class="glance-n">${rehearsal ? 'REHEARSAL' : `SOL ${ddd(n)}`}</span>
        <h2>${esc(longDate(r.date))}</h2>
      </div>
      <span class="badge ${rehearsal ? 'warn' : state === 'today' ? 'mars' : ''}">${rehearsal ? 'Before the run' : state === 'today' ? 'Today' : state === 'past' ? 'Complete' : 'Planned'}</span>
    </header>
    ${rehearsal ? `<p class="note" style="margin:0 0 14px">A preview, not the record: a run day's page as it will look, filled with
    what there is <b>today</b> — the habitat's readings as the sensors are sending them now, the plan for SOL 001
    (schedule, meals, consumption, power), whatever the crew have already written into the opening day, and any
    states filed today. This page disappears on 15 October, when SOL 001 takes its place.</p>` : ''}
    <div class="spec glance-spec">
      <span><b>${written.length}</b> crew entries</span>
      <span><b>${r.messages.length}</b> exchanges</span>
      <span><b>${r.traffic.sent}</b> messages from Earth</span>
      <span><b>${r.media.length}</b> media sent out</span>
    </div>`;
  ...
  const blog = written.length ? `
    <div class="glance-block">
      ...
      <div class="cards">${written.map((e) => `<article class="card">
        ...
        <div class="card-body entry-post">${MV.entryHtml(e.body, (r.media || []).filter((x) => x.crew_id === e.crew_id), { lookup: mediaLookup })}</div>
      </article>`).join('')}</div>
    </div>` : (state !== 'planned' ? `<div class="glance-block">${eyebrow('Blogs')}<div class="empty">No blog written by the crew on this day</div></div>` : '');

  const meals = day && day.meals.length ? `
    <div class="glance-block">
      ${eyebrow(`Meals${state === 'planned' ? ' · planned' : ''} · ${day.kcalPlanned} kcal · ${day.waterPlanned.toFixed(1)} L water · ${day.energyPlanned} Wh`)}
      <div class="tw"><table>
        <thead><tr><th>Slot</th><th>Meal</th><th class="n">kcal</th><th class="n">Water L</th><th class="n">Wh</th></tr></thead>
        <tbody>${day.meals.map((x) => `<tr><th>${esc(slotName[x.slot] || x.slot)}</th><td>${esc(x.name)}</td>
  ...
  const consumption = day && day.inventory.length ? `
    <div class="glance-block">
      ${eyebrow(`Consumption${state === 'planned' ? ' · planned' : ''} · at the close of the day · what is left of what was carried in`)}
      ${inventoryGauges(day.inventory, { cells: true })}
      <div class="tw"><table>
        <thead><tr><th>Store</th><th class="n">Remaining</th><th class="n">Used that day</th><th class="n">Days left</th></tr></thead>
  ...
  const power = pd.filed ? `
    <div class="glance-block">
      ${eyebrow(`Power consumed${state === 'planned' ? ' · planned' : ''} · ${pd.total.toFixed(2)} kWh`)}
      ...
      <div class="pwr-total"><span>Day total</span><b>${pd.total.toFixed(2)} kWh</b></div></div>
    </div>` : (state !== 'planned' ? `<div class="glance-block">${eyebrow('Power consumed')}<div class="empty">Not counted for this day</div></div>` : '');
  ...
  // own ingest recorded.
  const nodeTiles = (r.node || []).map((x) => { const [label, unit, dec] = NODE_LABELS[x.key] || [x.key, '', 1]; return tile(label, unit, x.av, x.lo, x.hi, x.n, dec); });
  const ingestTiles = r.habitat.map((h) => tile(h.label || h.metric, h.unit || '', h.avg_value, h.min_value, h.max_value, h.samples, 1));
  ...
  const figTile = (label, unit, v) => (v == null ? '' : `<div class="glance-tile">
      <span class="gt-label">${esc(label)}</span>
      <span class="gt-value">${Number(v).toLocaleString('en-GB')}<em>${esc(unit)}</em></span>
      <span class="gt-range">crew total · counted that day</span>
    </div>`);
  ...
  const charts = (r.series || []).map((s) => {
    ...
    return dayChart(s, r.dayStart, label, unit, dec, m.timezone);
  }).join('');
  const habitat = (nodeTiles.length || ingestTiles.length || figTiles) ? `
    <div class="glance-block">
      ${eyebrow(rehearsal ? 'Habitat · today, as the sensors are seeing it' : 'Habitat · the day as the sensors saw it, and as the crew counted it')}
      <div class="glance-tiles">${nodeTiles.join('')}${ingestTiles.join('')}${figTiles}</div>
      ${charts ? `<div class="gc-note">Every reading the station pulled or received that day, point by point, across the day (habitat time).</div>
      <div class="glance-charts">${charts}</div>` : ''}
    </div>` : (state !== 'planned' ? `<div class="glance-block">${eyebrow('Habitat')}<div class="empty">No readings on this day</div></div>` : '');

  const states = lastMood.size ? `
    <div class="glance-block">
      ${eyebrow('Crew condition · as reported, never as numbers')}
      <div class="rows">${[...lastMood.values()].map((s) => { const t = mood.translate(s); return `
        <div class="row"><div class="t">${esc(t.condition)}</div>
          <div class="m"><b>${esc(s.designation)}</b><span>${esc(t.lines.join('; '))}${s.activity ? ` — ${esc(s.activity)}` : ''}</span></div>
        </div>`; }).join('')}</div>
    </div>` : '';

  const exchanges = r.messages.length ? `
    <div class="glance-block">
      ${eyebrow('Exchanges with Earth')}
      <div class="cards">${r.messages.map((x) => `<article class="card">
        ...
        ${x.response_body ? `<div class="card-reply"><div class="who">${esc(x.responder || 'Mars habitat')}</div><p>${esc(x.response_body)}</p></div>` : ''}
      </article>`).join('')}</div>
    </div>` : '';

  const schedule = day && day.tasks.length ? `
    <div class="glance-block">
      ${eyebrow(`Schedule${state === 'planned' ? ' · planned' : ` · ${day.tasks.filter((t) => t.status === 'DONE').length}/${day.tasks.length} done`}`)}
      <div class="rows">${day.tasks.map((t) => `
        <div class="row ${t.status === 'DONE' ? 'done' : ''}"><div class="t">${esc(t.time)}</div>
          ...
          ${state !== 'planned' && t.status !== 'PLANNED' ? `<span class="badge">${esc(t.status)}</span>` : ''}
        </div>`).join('')}</div>
    </div>` : '';
  ...
  const reportBlock = (label, list) => (list.length ? `
    <div class="glance-block">
      ${eyebrow(label)}
      ${list.map((x) => `<div class="entry-post">${MV.entryHtml(x.body, [], { lookup: mediaLookup })}</div>`).join('')}
    </div>` : '');
  ...
  const notesBlock = notes.length ? `
    <div class="glance-block">
      ...
      <div class="rows">${notes.map((x) => `
        <div class="row"><div class="t">${esc(x.kind)}</div>
          <div class="m entry-post">${MV.entryHtml(x.body, [], { lookup: mediaLookup })}</div></div>`).join('')}</div>
    </div>` : '';
  ...
  const mediaBlock = loose.length ? `
    <div class="glance-block">${eyebrow('Also sent out')}${MV.strip(loose)}</div>` : '';
  ...
  if (rehearsal) {
    return `<div class="bk-page" id="today" data-day="0" role="group" aria-roledescription="page" aria-label="Rehearsal · today, before the run">${
      panel('REHEARSAL · NOT THE RECORD', body, 'mars-side glance-day')
    }</div>`;
  }
  return `<div class="bk-page" id="day-${n}" data-day="${n}" role="group" aria-roledescription="page" aria-label="SOL ${ddd(n)} · ${esc(longDate(r.date))}">${
...
function page(ctx, { records, rehearsal = null }) {
  ...
  <div style="padding:30px 0 10px">
    <div class="eyebrow">Channel group 30 · The whole mission, day by day</div>
    <h1>At a Glance</h1>
    <p class="lede">A booklet of the run: one day per page. Scroll or swipe sideways — or use the
    arrows — to turn to the next day. Each page holds everything its day held: the crew's blog,
    the exchanges with Earth, the schedule, the meals, the consumption, the habitat and the
    crew's condition. Days ahead show the plan.</p>
    <nav class="filters daypick" id="bk-strip" aria-label="Jump to a day">${
      (rehearsal ? `<a href="#today" data-day="0" class="${openOn === 0 ? 'on' : ''}" title="Today, before the run — a rehearsal preview">NOW</a>` : '')
    }${records.map((r) =>
  ...
  <div class="booklet-shell">
    <button type="button" class="bk-arrow prev" id="bk-prev" aria-label="Previous day">&#8249;</button>
    <div class="booklet" id="booklet" tabindex="0" aria-roledescription="carousel" aria-label="The days of the run">
      ${rehearsal ? daySection(rehearsal, m, { rehearsal: true }) : ''}${records.map((r) => daySection(r, m)).join('')}
    </div>
    <button type="button" class="bk-arrow next" id="bk-next" aria-label="Next day">&#8250;</button>
  </div>
  <div class="bk-counter" id="bk-counter" aria-live="polite">${openOn === 0 ? 'REHEARSAL · TODAY, BEFORE THE RUN' : `SOL ${String(openOn).padStart(3, '0')} / ${String(m.totalDays).padStart(3, '0')}`}</div>
  <script>
  (function () {
