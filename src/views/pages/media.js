...
function figure(m, { link = true } = {}) {
  ...
  else inner = `<span class="entry-doc">${esc(m.kind)} · <a href="${src}?download" download="${esc(m.filename)}">download</a></span>`;
...
function gallery(ctx, { days, counts, crew, filters }) {
  ...
  <div class="logpage-top">
    <div class="eyebrow">Channel group 60 · Media <span class="brk">Out of the habitat</span></div>
    <h1>Media</h1>
    <p class="lede">What the crew send out: photographs, video, sound. Every file is the original as it left the
    habitat — nothing re-encoded, nothing resized — kept under its own checksum, and every one of them can be
    downloaded, singly or all at once. ${counts.total} item${counts.total === 1 ? '' : 's'}${counts.total ? ` · ${counts.images} photograph${counts.images === 1 ? '' : 's'} · ${counts.videos} video${counts.videos === 1 ? '' : 's'} · ${counts.audio} recording${counts.audio === 1 ? '' : 's'} · ${fmtBytes(counts.bytes)}` : ''}.</p>
    <div class="actions">
      <a class="btn" href="/#media">Back to the mission</a>
      ${counts.total ? `<a class="btn primary" href="/media/export.zip">Download everything · ZIP · ${fmtBytes(counts.bytes)}</a>
      <a class="btn" href="/media/manifest.json">Manifest · every file with its SHA-256</a>` : ''}
    </div>
  </div>

  <div class="feed-filter log-filter logpage-filter" role="group" aria-label="Filter the media">
    ${chip('ALL', q(null, filters.crewId), !filters.kind)}
    ${chip('PHOTOGRAPHS', q('image', filters.crewId), filters.kind === 'image')}
    ${chip('VIDEO', q('video', filters.crewId), filters.kind === 'video')}
    ${chip('SOUND', q('audio', filters.crewId), filters.kind === 'audio')}
    ${chip('DOCUMENTS', q('document', filters.crewId), filters.kind === 'document')}
    <span class="chip-gap"></span>
    ${chip('ALL CREW', q(filters.kind, null), !filters.crewId)}
  ...
  ${days.length ? `<div class="logpage-days">${days.map((d) => `
    <section class="logpage-day media-day" id="day-${d.missionDay}">
      <div class="log-day-head">
        <span class="cs">Day ${dd(d.missionDay)}</span><span>${esc(missionLib.shortDay(d.date))}</span>
        ${!(m.phase === 'PRE_LAUNCH') && d.missionDay === m.clampedDay ? '<span class="now">Today</span>' : ''}
        <span class="count">${d.items.length} item${d.items.length === 1 ? '' : 's'} · <a href="/media/day/${d.missionDay}/export.zip">download the day</a></span>
      </div>
      <div class="mgrid">${d.items.map((x) => tile(x)).join('')}</div>
    </section>`).join('')}</div>`
  : `<div class="empty" style="padding:40px">Nothing has been sent out yet${m.phase === 'PRE_LAUNCH' ? ` — the habitat is occupied from ${esc(m.startLabel)}` : ''}.</div>`}`;

  return L.page({ title: 'Media', ctx, body, current: '/media',
    hero: L.masthead(ctx) + L.pageNav('/media'), hideRail: true, hideNav: true, bodyClass: 'landing inner' });
}
...
function player(m) {
  ...
  if (m.kind === 'video') return `<video class="mview-video" controls playsinline preload="metadata"${m.thumb_sha256 ? ` poster="${media.thumbUrl(m)}"` : ''}>
      ...
      This browser cannot play this file in the page — <a href="${src}?download" download="${esc(m.filename)}">download it</a> and open it locally.
    </video>
    <p class="note mview-note">If it will not play here, the file is still whole: download it and open it in any player.</p>`;
  ...
  return `<div class="empty" style="padding:40px">${esc(m.filename)} · ${fmtBytes(m.bytes)} — <a href="${src}?download" download="${esc(m.filename)}">download</a></div>`;
}

function item(ctx, { item: m, date, prev, next, position }) {
  const body = `
  <div class="logpage-top mview-top">
    <div class="eyebrow">Channel group 60 · Media · Day ${dd(m.mission_day)} · ${esc(missionLib.dayLabel(date))} <span class="brk">${position.i} of ${position.n} that day</span></div>
    <h1>${esc(m.caption || m.filename)}</h1>
    <p class="lede">${m.designation ? esc(m.designation) : 'Habitat'} · ${esc(m.kind)} · ${fmtBytes(m.bytes)}${m.width && m.height ? ` · ${m.width}×${m.height}` : ''}${m.duration_s ? ` · ${fmtDur(m.duration_s)}` : ''}${m.taken_at ? ` · made ${esc(m.taken_at)}` : ''}</p>
    <div class="actions">
      <a class="btn primary" href="${media.fileUrl(m)}?download" download="${esc(m.filename)}">Download the original · ${esc(m.filename)}</a>
      <a class="btn" href="/media#day-${m.mission_day}">All media</a>
      <a class="btn" href="/logbook#day-${m.mission_day}">That day's log</a>
      ${prev ? `<a class="btn" href="${media.pageUrl(prev)}">← Previous</a>` : ''}
      ${next ? `<a class="btn" href="${media.pageUrl(next)}">Next →</a>` : ''}
    </div>
  </div>
  <div class="mview">${player(m)}</div>
  <dl class="kv mview-kv">
    <dt>FILE</dt><dd>${esc(m.filename)} · ${esc(m.mime)}</dd>
    <dt>SIZE</dt><dd>${m.bytes.toLocaleString('en-GB')} bytes</dd>
    <dt>SHA-256</dt><dd class="mono">${esc(m.sha256)}</dd>
    <dt>SENT</dt><dd>${esc(m.uploaded_at)}</dd>
    <dt>DAY</dt><dd>Mission day ${dd(m.mission_day)} · ${esc(missionLib.dayLabel(date))}</dd>
    ${m.caption ? `<dt>CAPTION</dt><dd>${esc(m.caption)}</dd>` : ''}
  </dl>`;
