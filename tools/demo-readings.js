...
const start = require('../src/lib/run').dates().start;
...
if (t0 > now) {
  ...
  console.log('[demo]   MISSION_OVERRIDE=true MISSION_START=$(date -d "-4 days" +%F) MISSION_END=$(date -d "+8 days" +%F) npm run seed');
  process.exit(0);
}
